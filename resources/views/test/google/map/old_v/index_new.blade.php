...
<html>
	<head>
		...
		<style>
			...
			#csvBlock, #optionalBlock, #phoneListBlock, #submitBlock, #dbBlock{
				margin:10px;
			}
...
//			};
		...
		function addShowingPhoneList(){
			if(method !== undefined){
				if(method === "single"){
					$('input[type=radio][name=phone]:checked').each(function() {console.log(phoneList[this.value]);
		...
		function drawingAllRecordInfo(phoneIndex, recordIndex, delayTime){
			if(nextList[phoneIndex] === true){
				window.setTimeout(function() {
					...
					googleMap.addMarker(recordList[recordIndex], color);
