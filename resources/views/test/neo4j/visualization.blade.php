...
<html>
		...
		function D3(jsonData, cypher){
				...
				jsonData.links.forEach(function(link) {
						...
						link = duplicateRelationship(link);
				...
				function tick() {
					...
					line.attr("d", function(d) {
						...
						d.oriXY = -1; // if this.ori !== (-1), then we have to know the orientation for shift the text, if absX > absY, then the value of abxXY === 1, which means the text is shift in Y 
						...
						var mean = 0;
						var absDuplicate = 0;
						var absX = Math.abs(d.source.x-d.target.x);
						var absY = Math.abs(d.source.y-d.target.y);
						if(duplicateNum > 1){
								if(duplicateNum%2 === 1){
				...
				function lineClick(d){
					...
					var info = "<"+subjectOrigId+"> <"+d.type+"> <"+objectOrigId+">.";
						$("#node-info-box").html("<br>"+tablePathProperty(d, d.type, info));
				}
				...
				function duplicateRelationship(link){	
...
//			var svgId = "svg_"+document.getElementsByTagName("svg").length;
		...
		function tablePathProperty(object, type, info){
				var result = '<table class="table">\n\
								<thead><tr> <td class="col-md-4">Property</td> <td class="col-md-8">Value</td> </tr></thead>';
				result += '<tbody><tr><td class="col-md-4">ID</td><td class="col-md-8">'+object.originalId+'</td></tr>';
				result += '<tr><td class="col-md-4">Type</td><td class="col-md-8">'+type+'</td></tr>';
				result += '<tr><td class="col-md-4">RDF-Triple</td><td class="col-md-8">'+info+'</td></tr>';
				for (var key in object.properties) {
					 result += '<tr><td class="col-md-4">'+key+'</td><td class="col-md-8">'+object.properties[key]+'</td></tr>';
				}
				return result+"</tbody></table>";
